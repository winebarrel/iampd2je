...
- Only `data.aws_iam_policy_document.<name>.json` references are rewritten. If a policy is also referenced via something else (`.minified_json`, `.override_json`, etc.), the policy is left in place and a warning is printed to stderr.
- `source_policy_documents` and `override_policy_documents` are not supported; the surrounding data block is left in place with a stderr warning so the merge can be re-done by hand.
